...
desk_order_conversion = [
    'Team Elly', 'Team Vincent', 'Team Rahul', 'Team Sameer', 'Team Eden', 'Team Elena', 'Team Larisa', 'Team Myles'
]
...
desk_order_retention = [
    'Japan Team', 'Korean Team', 'Aarav Team', 'Ajay Team', 'French', 
    'AKA Team', 'Spanish', 'Portuguese'
]
desk_order_retention_dict = {desk: idx for idx, desk in enumerate(desk_order_retention)}

# Define team leaders for each team
team_leaders_conversion = {
    'Team Vincent': 'VINCENT MORRIS',
    'Team Rahul': 'RAHUL KUMAR',
    'Team Sameer': 'SAMEER SINGH',
    'Team Eden': 'EDEN MURPHY',
    'Team Elena': 'ELENA BERNARD',
    'Team Larisa': 'LARISA FRANCO',
    'Team Myles': 'MYLES JAMES'
}

team_leaders_retention = {
    'Aarav Team': 'AARAV SINGH',
    'Ajay Team': 'AJAY KUMAR',
    'French': 'MAXIME HULOT',
    'AKA Team': 'ALEX AKA',
    'Spanish': 'ANDRES MONTENEGRO',
    'Portuguese': 'PEDRO SILVA'
}

# Function to check if an agent is a team leader
def is_team_leader(row, team_leaders_dict):
    return row['Agent Name'] == team_leaders_dict.get(row['Desk'], '')

# Add a column to indicate if an agent is a team leader
agents_data['Is Team Leader'] = agents_data.apply(lambda row: is_team_leader(row, team_leaders_conversion) if row['DEPARTMENT'] == 1 else is_team_leader(row, team_leaders_retention), axis=1)
...
merged_data_conversion.sort_values(by=['Is Team Leader', 'Desk Order', 'Target Numeric'], ascending=[False, True, False], inplace=True)
...
merged_data_retention.sort_values(by=['Is Team Leader', 'Desk Order', 'Target Numeric'], ascending=[False, True, False], inplace=True)

# Drop the temporary 'Desk Order' column used for sorting
merged_data_conversion.drop(columns=['Desk Order', 'Target Numeric', 'DEPARTMENT', 'Is Team Leader'], inplace=True)
merged_data_retention.drop(columns=['Desk Order', 'Target Numeric', 'DEPARTMENT', 'Is Team Leader'], inplace=True)
